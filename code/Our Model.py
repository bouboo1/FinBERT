...
class RCNN(nn.Module):
    def __init__(self, bert_model, hidden_size=768, num_labels=3, dropout=0.1):
        ...
        self.conv = nn.Conv1d(hidden_size * 2, hidden_size, kernel_size=2, padding=1)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        conv_output = self.conv(lstm_output.permute(0, 2, 1))
...
# Attack对抗训练函数
def adversarial_training(model, input_ids, attention_mask, labels, epsilon=1e-5):
    ...
    conv_output = model.conv(lstm_output.permute(0, 2, 1))
    conv_output = torch.relu(conv_output)
    pooled_output = torch.max(conv_output, 2)[0]
